---
layout: homepage
---

## About Me

Hi! I am Qi Wang (王奇). I received my M.E. in Software Engineering from <a href="https://www.xidian.edu.cn/">Xidian University</a> and my B.E. in Communication Engineering from <a href="https://www.zjut.edu.cn/">Zhejiang University of Technology</a>. My recent research interests include Embodied AI, LLMs, text-to-3D generation, event-based vision, and multimodal human-computer interaction.

I enjoy building research systems that connect robust engineering with clear empirical analysis, especially for problems where visual perception, generative modeling, and real-world interaction meet.

## Research Interests

- Embodied AI and LLM-based agents
- Text-to-3D generation and score distillation
- Event-based vision and neuromorphic action recognition
- Multimodal gesture recognition and human-computer interaction

{% include projects.md %}

## Work Experience

- \[2023.05 - 2023.10\]: **Algorithm Engineer**, China Institute of Marine Technology & Economy, Beijing, China
  - Led algorithm development for gesture-based human-machine interaction in ship operation scenarios.
  - Collected skeletal and infrared gesture data with Leap Motion sensors and PyQt.
  - Built a TL-GCN based dynamic gesture recognition system with real-time consumer-producer integration.

## Selected Projects

- \[2021.03 - 2021.08\]: **National University Students Smart Car Race**, National First Prize
  - Team leader; achieved runner-up among 180 teams.
  - Built an autonomous vehicle system using STM32, NVIDIA Jetson Nano, ROS, obstacle avoidance, traffic-light recognition, and Dijkstra/A-star path planning.
- \[2023.10 - 2024.02\]: **Du RPC**, a scalable Java RPC framework
  - Designed a Dubbo-inspired RPC framework with Spring Boot Starter, Vert.x TCP networking, Etcd service discovery, SPI extensions, load balancing, retries, and fault tolerance.

## Honors and Awards

- Outstanding Graduate Student, 2023 - 2025
- Outstanding Graduate Student Leader, 2023 - 2025, Top 5%
- National First Prize, National University Students Smart Car Race, 2021
- National First Prize, 2nd Aerospace Cup Mobile Robot AI Innovation Technology Challenge, 2021
- Second Prize, Zhejiang Province University Student Mobile Communication 5G Technology Competition, 2021

## Education

- \[2022.09 - 2025.06\]: M.E., Software Engineering, <a href="https://www.xidian.edu.cn/">Xidian University</a>, China
- \[2018.09 - 2022.06\]: B.E., Communication Engineering, <a href="https://www.zjut.edu.cn/">Zhejiang University of Technology</a>, China

## Skills

- Programming: Python, C++, C, Java, MATLAB
- Tools: PyTorch, STM32, ROS, Altium Designer
- Languages: Mandarin, English

{% include visitors.md %}
